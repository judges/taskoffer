...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
	...
	[self scheduleBlock:^{
		...
		else
		{
            ...
            NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:[messageBody dataUsingEncoding:NSUTF8StringEncoding] options:(NSJSONReadingMutableLeaves) error:nil];
            if (dict != nil && [dict objectForKey:@"projectID"] != nil)
            {
#warning 此处的作用是判断如果信息是项目接包时的消息，那么则不向最近聊天信息表里写入内容，这样的结果就是不能找到项目聊天时的最近聊天内容
                return ;
            }
			if ([messageBody length] > 0)
			{
