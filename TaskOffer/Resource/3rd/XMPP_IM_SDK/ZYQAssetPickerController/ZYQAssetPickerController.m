...
+ (NSDateComponents *)componetsWithTimeInterval:(NSTimeInterval)timeInterval{
    ...
    NSDate *toDate = [[NSDate alloc] initWithTimeInterval:timeInterval sinceDate:fromDate];
    
    unsigned int calendarUnit = NSSecondCalendarUnit | NSMinuteCalendarUnit | NSHourCalendarUnit | NSDayCalendarUnit | NSMonthCalendarUnit | NSYearCalendarUnit;
...
-(void)drawRect:(CGRect)rect{
    ...
    CGRect textRect = CGRectMake(rect.size.width - titleSize.width - 2, (height - 12) * 0.5, kThumbnailLength, kThumbnailLength);
    [self.text drawWithRect:textRect options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : self.font} context:nil];
    
    UIImage *image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"ZYQAssetPicker.Bundle/Images/AssetsPickerVideo@2x.png"]];
    [image drawAtPoint:CGPointMake(2, (height - image.size.height) * 0.5)];
}
...
+ (void)initialize{
    checkedIcon = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"ZYQAssetPicker.Bundle/Images/%@@2x.png",(!isIOS7) ? @"AssetsPickerChecked~iOS6" : @"AssetsPickerChecked"]]];
...
- (id)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]){
        ...
        _videoTitle = [[ZYQVideoTitleView alloc] initWithFrame:CGRectMake(0, kThumbnailSize.height - titleHeight, kThumbnailSize.width, titleHeight)];
...
- (void)bind:(NSArray *)assets selectionFilter:(NSPredicate*)selectionFilter minimumInteritemSpacing:(float)minimumInteritemSpacing minimumLineSpacing:(float)minimumLineSpacing columns:(NSInteger)columns assetViewX:(float)assetViewX{
    ...
    if (count < assets.count) {
        for (NSInteger i = 0; i < assets.count; i ++) {
            if (i > count - 1) {
                ...
                ZYQAssetView *assetView = self.contentView.subviews[i];
                assetView.frame = CGRectMake(assetViewX + (kThumbnailSize.width + minimumInteritemSpacing) * i, minimumLineSpacing - 1, kThumbnailSize.width, kThumbnailSize.height);
                [assetView bind:assets[i] selectionFilter:selectionFilter isSeleced:[((ZYQAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i]]];
            }
        }
    }else{
        for (NSInteger i = count; i > 0; i --) {
            ZYQAssetView *assetView = self.contentView.subviews[i - 1];
            if (i > assets.count) {
                [assetView removeFromSuperview];
            }else{
                assetView.frame = CGRectMake(assetViewX + (kThumbnailSize.width + minimumInteritemSpacing) * (i - 1), minimumLineSpacing - 1, kThumbnailSize.width, kThumbnailSize.height);
                [assetView bind:assets[i - 1] selectionFilter:selectionFilter isSeleced:[((ZYQAssetViewController*)_delegate).indexPathsForSelectedItems containsObject:assets[i - 1]]];
            }
        }
    }
}
...
- (id)init{
    ...
    if(UIInterfaceOrientationIsLandscape(self.interfaceOrientation)){
        self.tableView.contentInset = UIEdgeInsetsMake(0.0, 2.0, 0.0, 2.0);
...
- (void)setupButtons{
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"完成", nil) style:UIBarButtonItemStylePlain target:self action:@selector(finishPickingAssets:)];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == ceil(self.assets.count * 1.0 / columns)) {
        ...
        if (_numberOfVideos == 0){
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%ld 张照片", nil), (long)_numberOfPhotos];
        }else if (_numberOfPhotos == 0){
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%ld 部视频", nil), (long)_numberOfVideos];
        }else{
            cell.textLabel.text = [NSString stringWithFormat:NSLocalizedString(@"%ld 张照片, %ld 部视频", nil), (long)_numberOfPhotos, (long)_numberOfVideos];
        }
        return cell;
    }
    ...
    cell.delegate = self;
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:(self.tableView.frame.size.width - kThumbnailSize.width * tempAssets.count - minimumInteritemSpacing * (tempAssets.count - 1)) * 0.5];
    return cell;
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return ceil(self.assets.count * 1.0 / columns) + 1.0;
}
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset{
    ZYQAssetPickerController *assetPickerController = (ZYQAssetPickerController *)self.navigationController;
    BOOL selectable = [assetPickerController.selectionFilter evaluateWithObject:asset];
    if (_indexPathsForSelectedItems.count >= assetPickerController.maximumNumberOfSelection && ![_indexPathsForSelectedItems containsObject:asset]) {
        if (assetPickerController.delegate != nil && [assetPickerController.delegate respondsToSelector:@selector(assetPickerControllerDidMaximum:)]) {
            [assetPickerController.delegate assetPickerControllerDidMaximum:assetPickerController];
        }
    }
    
    return (selectable && _indexPathsForSelectedItems.count < assetPickerController.maximumNumberOfSelection);
}
...
- (void)didSelectAsset:(ALAsset *)asset{
    ...
    ZYQAssetPickerController *assetPickerController = (ZYQAssetPickerController *)self.navigationController;
    assetPickerController.indexPathsForSelectedItems = _indexPathsForSelectedItems;
    
    if (assetPickerController.delegate != nil && [assetPickerController.delegate respondsToSelector:@selector(assetPickerController:didSelectAsset:)]){
        [assetPickerController.delegate assetPickerController:assetPickerController didSelectAsset:asset];
    }
...
- (void)didDeselectAsset:(ALAsset *)asset{
    ...
    if (assetPickerController.delegate != nil && [assetPickerController.delegate respondsToSelector:@selector(assetPickerController:didDeselectAsset:)]){
        [assetPickerController.delegate assetPickerController:assetPickerController didDeselectAsset:asset];
    }
    [self setTitleWithSelectedIndexPaths:_indexPathsForSelectedItems];
}
...
- (void)finishPickingAssets:(id)sender{
    if(_indexPathsForSelectedItems.count > 0){
        ...
        for (ALAsset *asset in _indexPathsForSelectedItems){
            UIImage *image = [UIImage imageWithCGImage:asset.defaultRepresentation.fullScreenImage];
            if(image != nil){
                [selectedImages addObject:UIImageJPEGRepresentation(image, 1.0)];
            }
        }
        
        ZYQAssetPickerController *assetPickerController = (ZYQAssetPickerController *)self.navigationController;
        if(assetPickerController.delegate != nil && [assetPickerController.delegate respondsToSelector:@selector(assetPickerController:didFinishPickingAssets:)]){
            [assetPickerController.delegate assetPickerController:assetPickerController didFinishPickingAssets:[selectedImages copy]];
        }
        
        if (assetPickerController.isFinishDismissViewController) {
            [assetPickerController.presentingViewController dismissViewControllerAnimated:YES completion:nil];
        }
    }else{
        UIAlertView *alertView = [[UIAlertView  alloc] initWithTitle:@"" message:@"您未选择图片！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alertView show];
    }
}

- (void)removeSelectedItemAtIndex:(NSUInteger)index{
    if(index < _indexPathsForSelectedItems.count){
        [_indexPathsForSelectedItems removeObjectAtIndex:index];
        [self.tableView reloadData];
    }
}
...
- (void)bind:(ALAssetsGroup *)assetsGroup{
    ...
    CGFloat scale = height / kThumbnailLength;
...
@property (nonatomic, strong) ZYQAssetViewController *assetViewController;
...
- (void)setupViews{
    self.tableView.rowHeight = kThumbnailLength + 12.0;
...
- (void)setupButtons{
    ...
    if (assetPickerController.showCancelButton){
...
- (void)setupGroup{
    ...
    ZYQAssetPickerController *assetPickerController = (ZYQAssetPickerController *)self.navigationController;
    ALAssetsFilter *assetsFilter = assetPickerController.assetsFilter;
    
    ALAssetsLibraryGroupsEnumerationResultsBlock resultsBlock = ^(ALAssetsGroup *group, BOOL *stop) {
        if (group){
            [group setAssetsFilter:assetsFilter];
            if (group.numberOfAssets > 0 || assetPickerController.showEmptyGroups){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    _assetViewController = [[ZYQAssetViewController alloc] init];
    _assetViewController.assetsGroup = [self.groups objectAtIndex:indexPath.row];
...
- (void)removeSelectedItemAtIndex:(NSUInteger)index{
    [self.assetViewController removeSelectedItemAtIndex:index];
}
...
- (id)init{
    ZYQAssetGroupViewController *assetGroupViewController = [[ZYQAssetGroupViewController alloc] init];
    
    if (self = [super initWithRootViewController:assetGroupViewController]){
        ...
        self.navigationBar.titleTextAttributes = @{NSFontAttributeName : [UIFont boldSystemFontOfSize:19.0],NSForegroundColorAttributeName : [UIColor whiteColor]};
        self.navigationBar.barTintColor = [UIColor colorWithHexString:kDefaultBarColor];
    }
    return self;
}

- (void)removeSelectedAssetAtIndex:(NSUInteger)index{
    ZYQAssetGroupViewController *assetGroupViewController = (ZYQAssetGroupViewController *)self.topViewController;
    [assetGroupViewController removeSelectedItemAtIndex:index];
}
