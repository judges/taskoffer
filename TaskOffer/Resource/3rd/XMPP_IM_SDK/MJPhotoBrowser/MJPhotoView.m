    ...
    if (_photo.image) {
        ...
        [_imageView sd_setImageWithURL:_photo.url placeholderImage:_photo.srcImageView.image options:SDWebImageRetryFailed | SDWebImageLowPriority progress:^(NSInteger receivedSize, NSInteger expectedSize) {
            if (receivedSize > kMinProgress) {
                __strong MJPhotoLoadingView *photoLoadingView = loading;
                if(photoLoadingView){
                    photoLoadingView.progress = (float)receivedSize/expectedSize;
                }
            }
        } completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            __strong MJPhotoView *sPhotoView = photoView;
            if(sPhotoView){
                [sPhotoView photoDidFinishLoadWithImage:image];
            }
        }];
    }
}
...
- (void)hide
{
    ...
    [UIView animateWithDuration:duration + 0.1 animations:^{
        ...
        if(_photo.srcImageView.image == nil){
