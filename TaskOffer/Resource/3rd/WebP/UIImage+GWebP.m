...
+ (NSData *)imageToWebP:(UIImage *)image{
    return [self imageToWebP:image quality:75.0];
}
...
+ (void)imageToWebP:(UIImage *)image completionBlock:(void (^)(NSData *))completionBlock{
    [self imageToWebP:image quality:75.0 alpha:1.0 preset:WEBP_PRESET_DEFAULT completionBlock:^(NSData *result) {
        if(completionBlock){
